...
const attachComments = async <T extends { id: string }>(events: T[]) => {
	if (events.length === 0) {
		return [] as Array<
			T & { comments: Array<typeof schema.comment.$inferSelect> }
		>;
	}
	...
	const commentsByEventId = new Map<
		string,
		Array<typeof schema.comment.$inferSelect>
	>();
...
const attachEventEngagement = async <T extends { id: string }>(
	...
	if (events.length === 0) {
		return [] as Array<
			T & { attendeeCount: number; isGoing: boolean; isStarred: boolean }
		>;
	}
...
export const putEventDataFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data, context }) => {
		...
		const updatedEvent = await db
			...
			.where(
				and(eq(schema.event.id, data.id), eq(schema.event.userId, user.id)),
			)
			.returning();
...
export const updateCommentForEventFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data, context }) => {
		...
		const updatedComment = await db
			...
			.where(
				and(eq(schema.comment.id, data.id), eq(schema.comment.userId, userId)),
			)
			.returning();
