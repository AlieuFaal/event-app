		...
		<section className="space-y-3">
			<div className="flex items-center justify-between gap-4">
				...
				<div className="flex flex-wrap items-center justify-end gap-3">
					...
					<Link
						className="inline-flex items-center gap-2 font-semibold text-[var(--explore-purple-text)] text-sm transition hover:text-[var(--explore-heading)]"
						to="/event-calendar"
					>
						View full calendar
					...
					return (
						<button
							className={cn(
								...
								isSelected &&
									"border-[var(--explore-border-strong)] bg-[var(--explore-purple-soft)] shadow-[0_0_0_1px_rgba(191,115,255,0.25)]",
							)}
							key={day.toISOString()}
							onClick={() => onDateSelect(isSelected ? null : day)}
							type="button"
						>
							<p className="font-black text-[var(--explore-heading)] text-sm">
								{index === 0 || isToday
									? "Today"
									: day.toLocaleDateString("en-US", { weekday: "short" })}
							</p>
							<p className="text-[var(--explore-muted)] text-xs">
