		...
		<section className="space-y-3">
			<div className="flex items-center justify-between gap-4">
				...
				<div className="flex items-center gap-2">
					<span className="font-semibold text-[var(--explore-purple-text)] text-sm">
						{canCycleEvents
			...
			<div className="min-h-[238px] overflow-hidden rounded-[10px] border border-[var(--explore-border)] bg-[var(--explore-card-strong)] shadow-[var(--explore-shadow)]">
				{events.length > 0 ? (
					visibleEvents.map((event) => {
						...
						return (
							<article
								className="group grid min-h-[79px] w-full cursor-pointer grid-cols-[54px_86px_minmax(0,1fr)] items-center gap-3 border-[var(--explore-border)] border-b p-3 text-left transition last:border-b-0 hover:bg-[var(--explore-purple-soft)] sm:grid-cols-[56px_92px_minmax(0,1fr)_auto]"
								key={event.id}
							...
							>
								<div className="rounded-[8px] border border-[var(--explore-border)] bg-[var(--explore-purple-soft)] px-2 py-2 text-center">
									<p className="font-black text-[11px] text-[var(--explore-muted)]">
										{dateBadge.day}
									</p>
									<p className="font-black text-[var(--explore-heading)] text-lg">
										{dateBadge.date}
									</p>
								</div>
								...
								<div className="min-w-0 space-y-1.5">
									<h3 className="truncate font-black text-[var(--explore-heading)] text-base">
										{event.title}
									</h3>
									<p className="truncate text-[var(--explore-muted)] text-sm">
										{getLocationLabel(event)} · {formatDateLine(startDate)} ·{" "}
										{formatTimeRange(startDate, endDate)}
									</p>
									<div className="flex flex-wrap items-center gap-2">
										<span
											className={`rounded-[7px] px-2.5 py-1 font-bold text-xs ${accent.bg} ${accent.text}`}
										>
											{event.genre ?? "Event"}
										</span>
									</div>
								...
								<div className="col-span-3 flex items-center justify-between gap-3 sm:col-span-1 sm:justify-end">
									<span className="inline-flex items-center gap-1.5 text-[var(--explore-text)] text-sm">
