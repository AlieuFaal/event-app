		...
		<section className="space-y-3">
			<div className="flex flex-wrap items-end justify-between gap-3">
				<div className="flex flex-wrap items-center gap-3">
					...
					<span className="text-[var(--explore-faint)] text-sm">
						...
						return (
							<article
								className={cn(
									...
									index === 0 &&
										"border-[#8f5cff]/55 shadow-[0_22px_70px_rgba(124,77,255,0.16)]",
								)}
								key={event.id}
							...
							>
								...
								<div className="relative flex min-h-[188px] flex-col justify-between p-4">
									<div className="flex items-start justify-between gap-3">
										<span className="rounded-[7px] bg-black/55 px-2.5 py-1 font-black text-white text-xs">
									...
									<div className="min-w-0 space-y-2">
										<div>
											<h3 className="truncate font-black text-white text-xl">
												{event.title}
											</h3>
											<p className="mt-1 flex items-center gap-1 truncate text-[#c5c0cf] text-xs">
												<MapPin className="size-3 text-[#a98cff]" />
												{getLocationLabel(event)}
											</p>
										</div>
										<span
											className={`inline-flex rounded-[7px] px-2.5 py-1 font-bold text-xs ${accent.bg} ${accent.text}`}
										>
											{event.genre ?? "Event"}
										</span>
									</div>
