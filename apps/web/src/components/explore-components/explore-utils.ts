...
const genreAccentByName: Record<string, { bg: string; text: string; dot: string }> = {
  Electronic: { bg: "bg-cyan-500/15", text: "text-cyan-200", dot: "bg-cyan-400" },
  House: { bg: "bg-blue-500/15", text: "text-blue-200", dot: "bg-blue-400" },
  Techno: { bg: "bg-sky-500/15", text: "text-sky-200", dot: "bg-sky-400" },
  "Hip-Hop": { bg: "bg-pink-500/15", text: "text-pink-200", dot: "bg-pink-400" },
  Indie: { bg: "bg-amber-500/15", text: "text-amber-200", dot: "bg-amber-400" },
  Jazz: { bg: "bg-yellow-500/15", text: "text-yellow-200", dot: "bg-yellow-400" },
  "R&B": { bg: "bg-rose-500/15", text: "text-rose-200", dot: "bg-rose-400" },
  Pop: { bg: "bg-fuchsia-500/15", text: "text-fuchsia-200", dot: "bg-fuchsia-400" },
  Rock: { bg: "bg-red-500/15", text: "text-red-200", dot: "bg-red-400" },
  Afrobeat: { bg: "bg-emerald-500/15", text: "text-emerald-200", dot: "bg-emerald-400" },
  Ambient: { bg: "bg-teal-500/15", text: "text-teal-200", dot: "bg-teal-400" },
};

export function getGenreAccent(genre?: string | null) {
  if (!genre) return { bg: "bg-violet-500/15", text: "text-violet-200", dot: "bg-violet-400" };
  return genreAccentByName[genre] ?? { bg: "bg-violet-500/15", text: "text-violet-200", dot: "bg-violet-400" };
}
...
export function getLocationLabel(event: Pick<ExploreEvent, "venue" | "address">) {
...
export function getGoogleMapsHref(event: ExploreEvent) {
  ...
  const query = latitude && longitude ? `${latitude},${longitude}` : event.address;
...
export function formatEndsIn(endDate: Date, now: Date) {
  const totalMinutes = Math.max(0, Math.round((endDate.getTime() - now.getTime()) / 60000));
...
export function formatStartsIn(startDate: Date, now: Date) {
  const totalMinutes = Math.max(0, Math.round((startDate.getTime() - now.getTime()) / 60000));
