...
export default function SignIn({ redirectTo = "/" }: { redirectTo?: string }) {
	...
	const handleSignIn = async () => {
		await authClient.signIn.email(
			...
			},
			{
				...
				onSuccess: async (ctx) => {
					const nextRoute =
						ctx.data?.user?.role === "New User" ? "/onboarding" : redirectTo;
	...
	return (
		<Card className="h-auto w-full max-w-3xl">
			<CardHeader>
			...
			<CardContent>
				<div className="grid gap-4">
					...
					>
						...
						>
							{/** biome-ignore lint/a11y/noSvgWithoutTitle: Decorative provider logo icon paired with adjacent text label. */}
							<svg
