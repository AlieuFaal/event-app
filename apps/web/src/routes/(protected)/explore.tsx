...
function ExploreComponent() {
	...
	const [attendancePendingEventId, setAttendancePendingEventId] = useState<
		string | null
	>(null);
	...
	const trendingEvents = useMemo(() => {
		const hasAttendance = filteredEvents.some(
	...
	const genreCounts = useMemo(
		() => getEventCountsByGenre(activeEvents),
		[activeEvents],
	);
	...
	const liveCount = activeEvents.filter((event) =>
	...
	const handleToggleAttendance = async (event: ExploreEvent) => {
		...
		try {
			...
			toast.error(
				error instanceof Error ? error.message : "Could not update RSVP",
			);
		} finally {
