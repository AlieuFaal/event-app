				...
				<CardContent className="w-full flex flex-row justify-around items-center p-1 gap-2 rounded-xl">
					<View className="w-32 h-28 justify-center items-center overflow-hidden rounded-sm border-1">
						<Image
							...
							source={event.imageUrl ? { uri: event.imageUrl } : randomImage()}
							resizeMode="cover"
						/>
					</View>
					<View className=" w-40 h-28 items-center rounded-2xl p-2 -left-2">
						<CardHeader className="w-full h-full justify-center items-center p-0 m-0">
							<CardTitle className="text-sm line-clamp-1 w-full text-gray-900 dark:text-white">
								{event.title}
							</CardTitle>
							<CardDescription className="text-sm line-clamp-1 text-gray-600 dark:text-gray-300">
								{event.description}
							</CardDescription>
							<CardDescription className="text-sm line-clamp-1 text-gray-600 dark:text-gray-300">
								{event.address}
