  ...
  const handleSubmit = async () => {
    ...
    try {
      if (!hasContent) {
        ...
        console.log(
          "No additional info provided, will not update. skipping to next step.",
        );
        return;
      }
      await apiClient.users.updateonboardinginfo[":id"].$put({
  ...
  return (
    <SafeAreaView className="flex-1" edges={["top", "bottom"]}>
      <View className="flex-1">
        <Card className="rounded-none flex-1 bg-transparent dark:bg-transparent px-0 border-transparent">
          <CardHeader className="flex flex-col items-center mt-14 gap-2 px-6">
            ...
            <CardDescription className="text-xl text-center mt-5">
              We just need a bit more info to set up your profile.
            </CardDescription>
          </CardHeader>
          ...
          >
            <CardContent className="px-6">
              <Label nativeID="phone" className="text-gray-900 dark:text-white">
                Phone number
              </Label>
              <View className="mt-2">
                <Input
